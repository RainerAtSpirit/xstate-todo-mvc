...
function filterTodos(state: any, todos: any) {
...
export function Todos() {
  ...
  useHashChange((_e: any) => {
    send(`SHOW.${window.location.hash.slice(2) || "all"}`);
  });

  const { todos, todo } = state.context;

  const numActiveTodos = todos.filter((todo: { completed: any; }) => !todo.completed).length;
    ...
    <section className="todoapp" data-state={state.toStrings()}>
      <header className="header">
        ...
        <input
          ...
          onKeyPress={e => {
            if (e.key === "Enter") {
              send({ type: "NEWTODO.COMMIT", value: (e.target as HTMLInputElement).value });
            }
          }}
          onChange={e =>
            send({
      ...
      <section className="main">
        ...
        <ul className="todo-list">
          {filteredTodos.map((todo: { id: string | number | undefined; }) => (
            <Todo
              key={todo.id}
              todo={todo}
              onChange={(todo: any) => send({ type: "TODO.COMMIT", todo })}
              onDelete={(id: any) => send({ type: "TODO.DELETE", id })}
            />
          ))}
        </ul>
      </section>
